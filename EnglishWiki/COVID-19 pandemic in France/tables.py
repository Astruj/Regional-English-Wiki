...
fr_articles = list(df['article'])
pageviews1 = list(df['Pageviews'])
numArticles1 = len(fr_articles)
temp = 0
...
filename = "fr_pgviewsPerEditorTable1.csv"
with open(filename, "w+") as f: 
    writer = csv.writer(f)
    writer.writerow(parameters)
    x = 0
    for i in range(0,fr_len):
        row = [fr_articles[i],ratio1[i]]
        x = x +ratio1[i]
        writer.writerow(row)
    row = ['Average', x/fr_len]
    writer.writerow(row)
...
filename = "fr_revisionsPerEditorTable1.csv"
with open(filename, "w+") as f: 
    writer = csv.writer(f)
    writer.writerow(parameters)
    x = 0
